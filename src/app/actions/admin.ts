...
import { adminEntitySchema, classSchema, subjectSchema, uuid } from "@/lib/validation";
...
export async function saveAdminRecord(_state: ActionState, form: FormData): Promise<ActionState> {
  try {
    ...
    if (entity.data === "classes") {
      ...
      return { error: "Используйте редактор PDF или импорт недельного расписания." };
    }
