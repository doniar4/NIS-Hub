...
export async function generateStudy(raw:unknown):Promise<StudyResult>{
 ...
 const parsed=studyInput.safeParse(raw);if(!parsed.success)return {error:"unavailable"};
 const input=parsed.data;
 if(input.end-input.start+1>config.maxPages)return {error:"unavailable"};
 ...
 try{
  ...
  if(edition.error||!edition.data||edition.data.publication_status!=="published"||(edition.data.page_count&&input.end>edition.data.page_count))return {error:"unavailable"};
  const book=await supabase.from("books").select("publication_status").eq("id",edition.data.book_id).maybeSingle();
  if(book.error||book.data?.publication_status!=="published")return {error:"unavailable"};
  ...
  if(extracted.error||extracted.data?.status!=="ready"||extracted.data.content_revision!==revision)return {error:"unavailable"};
  ...
  if(result.error||result.data.length!==input.end-input.start+1)return {error:"unavailable"};
  const chars=result.data.reduce((sum,p)=>sum+p.text.length,0);
  if(chars<80||chars>config.maxChars)return {error:"unavailable"};
  ...
  if(reservation.error)return {error:"unavailable"};
  const reserved=reservationSchema.parse(reservation.data);
  if(reserved.state==="busy"||reserved.state==="quota")return {error:reserved.state};
  const g=reserved.generation;if(!g)return {error:"failed"};
  ...
  if(reserved.state==="cached"){
   if(!g.response||!g.signature||!validCacheSignature(config.key,context,g.response,g.signature))return {error:"failed"};
  ...
  if(saved.error||!saved.data){await fail();return {error:"unavailable"};}
  return {response,cached:false,source};
 }catch(error){
  if(fail)await fail().catch(()=>{});
  return {error:error instanceof StudyProviderError?error.code:"failed"};
 }
}
