...
export default async function ReadPage({ params, searchParams }: {
    ...
    return <SiteShell><Link className="text-sm underline" href={`/books/${id}`}>{t.aboutMaterial}</Link><h1 className="section-title mt-6 mb-8">{book.title}</h1><EditionPicker bookId={id} variants={variants} selected={edition.id} locale={locale} reader/><div className="reader-workspace"><AiStudyPanel key={edition.id+"-ai"} variantId={edition.id} initialPage={Math.min(startPage,edition.page_count??1000)} totalPages={edition.page_count} config={{enabled:ai.enabled,maxPages:ai.maxPages,maxChars:ai.maxChars,dailyLimit:ai.dailyLimit}}/><PdfReader key={edition.id} bookId={id} variantId={edition.id} initialPage={Math.min(startPage, edition.page_count ?? 100000)} initialBookmarks={bookmarks.data.map(b => b.page_number)}/></div></SiteShell>;
}
