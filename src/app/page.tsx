...
export default async function Home() {
    ...
    return <SiteShell><PageIntro kicker={classes.find(item => item.id === classId)?.name ?? t.yourSpace} title={viewer.profile?.display_name ? t.hello + viewer.profile.display_name : t.yourDay}>{date}</PageIntro><div className="mt-10 grid gap-10 lg:grid-cols-2"><section className="border-t-2 border-[var(--ink)] pt-5"><h2 className="section-title">{t.today}</h2>{lessons.length ? <ol className="mt-4 divide-y divide-[var(--line)]">{lessons.map(lesson => <li className="flex items-center gap-4 py-4" key={lesson.class_id + lesson.date + lesson.lesson_number}><span className="text-[var(--muted)]">{lesson.lesson_number}</span><span className="flex-1">{subjectName(subjects.find(item => item.id === lesson.subject_id), locale)}</span>{lesson.room && <span>{lesson.room}</span>}</li>)}</ol> : <p className="py-6 text-[var(--muted)]">{classId ? t.noToday : t.chooseProfileClass}</p>}<SectionLink href="/schedule">{t.allSchedule}</SectionLink></section><section className="border-t-2 border-[var(--ink)] pt-5"><h2 className="section-title">{t.continueReading}</h2><ReadingList /><SectionLink href="/library">{t.openLibrary}</SectionLink></section></div></SiteShell>;
}
