...
export default async function TicketPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{page?:string}>}){
 ...
 return <SiteShell><div className="support-workspace support-detail"><aside className="surface-card support-list"><TicketList/></aside><section className="support-active"><TicketConversation ticket={ticket} messages={messages.data} events={events.data} locale={locale} admin={profile.role==="admin"} page={page} replyAction={replyTicket} statusAction={changeTicketStatus}/></section></div></SiteShell>;
}
