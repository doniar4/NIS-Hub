...
export default async function SupportPage({searchParams}:{searchParams:Promise<{status?:string;page?:string}>}){
 ...
 return <SiteShell><PageIntro title={t.support}/><div className="support-workspace mt-8"><section className="surface-card support-list"><h2 className="section-title">{t.support}</h2><TicketList status={params.status} page={Number(params.page??0)}/></section><section className="surface-card"><h2 className="section-title mb-5">{t.newTicket}</h2><ActionForm action={createTicket} label={t.send}><TicketFields locale={locale}/></ActionForm></section></div></SiteShell>;
}
