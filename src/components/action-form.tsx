...
export function ActionForm({ action, children, label, className = "space-y-5", disabled = false }: { action: FormAction; children?: ReactNode; label: string; className?: string; disabled?: boolean }) {
  ...
  return <form action={formAction} className={className}>
    <fieldset className="min-w-0 space-y-5" disabled={pending || disabled}>{children}
      <button className="button" disabled={pending || disabled} type="submit">{pending ? "Обработка…" : label}</button>
    </fieldset>
