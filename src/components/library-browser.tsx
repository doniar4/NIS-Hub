...
export function LibraryBrowser({ books, subjects, initial, truncated }: {
  books: LibraryBook[]; classes: ClassRow[]; subjects: SubjectRow[]; initial: LibraryFilters; truncated: boolean;
}) {
  const { t, locale } = useI18n(); const p=v051Copy(locale);
  ...
  if (previousInitial !== initial) {
    ...
    setQ(initial.q); setGrade(initial.grade); setSubject(initial.subject);
  }
  const filteredBooks = useMemo(() => filterBooks(books, { q, grade, subject })
    .sort((a, b) => a.title.localeCompare(b.title, locale)), [books, q, grade, subject, locale]);
  useEffect(() => {
    const restore = () => {
      const values = initialLibraryFilters(Object.fromEntries(new URLSearchParams(window.location.search)));
      setQ(values.q); setGrade(values.grade); setSubject(values.subject);
    };
  ...
  return <section aria-label={t.library}>
    <div role="search" className="mt-8 grid items-end gap-4 sm:grid-cols-2 lg:grid-cols-[minmax(8rem,1.7fr)_minmax(6rem,.7fr)_minmax(10rem,1fr)_auto]">
      <label><span className="field-label">{t.title}</span><input className="field" type="search" maxLength={100}
        value={q} onChange={event => change({ q: event.target.value, grade, subject })}/></label>
      <label><span className="field-label">{p.grade}</span><select className="field" value={grade}
        onChange={event => change({ q, grade: event.target.value, subject })}>
        <option value="">{p.allGrades}</option>
        {grade && !BOOK_GRADES.some(item => String(item) === grade) && <option value={grade}>{t.unavailableFilter}</option>}
        {BOOK_GRADES.map(item => <option value={item} key={item}>{item}</option>)}
      </select></label>
      <label><span className="field-label">{t.subject}</span><select className="field" value={subject}
        onChange={event => change({ q, grade, subject: event.target.value })}>
      ...
      <button className="button button-secondary" type="button" onClick={() => change({ q: "", grade: "", subject: "" })}>{t.resetFilters}</button>
    </div>
