...
export function TicketConversation({ticket,messages,events,locale,admin,page=0,replyAction,statusAction}:{
 ...
 return <><Link className="text-link" href={admin?"/admin/tickets":"/support"}>{t.support}</Link><PageIntro title={ticket.title}>{t.categories[ticket.category]} · <StatusChip tone={ticket.status==="resolved"?"success":ticket.status==="in_progress"?"warning":"neutral"}>{t.statuses[ticket.status]}</StatusChip></PageIntro>
