...
export type LessonSnapshot = Pick<WeeklyLesson,"id"|"class_id"|"weekday"|"lesson_start"|"lesson_end"|"start_time"|"end_time"|"subject_id"|"teacher"|"room"|"effective_from"|"effective_to"|"subgroup_key"|"subgroup_label"|"audience">;
export function scheduleDiff(before: LessonSnapshot[], after: LessonSnapshot[]) {
  const key=(row:LessonSnapshot)=>[row.class_id,row.weekday,row.lesson_start,row.effective_from??"",row.subgroup_key??""].join("/");
  ...
  const changed=after.flatMap(row=>{
    ...
    if(row.lesson_end!==previous.lesson_end||row.effective_to!==previous.effective_to||row.teacher!==previous.teacher||row.subgroup_label!==previous.subgroup_label||row.audience!==previous.audience)fields.push("block");
