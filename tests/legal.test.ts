...
test("all legal drafts contain current services, storage, persistence, operator and effective-date placeholders", () => {
  for (const copy of Object.values(legalCopy)) {
    assert.equal(copy.privacy.length, 9); assert.equal(copy.terms.length, 8);
    const privacy = copy.privacy.flat().join(" ");
    for (const term of ["Supabase", "Auth", "Storage", "<uid>/avatar.webp", "Top 4", "nis-theme", "nis-locale", "localStorage", "EMAIL"]) assert.ok(privacy.includes(term), term);
