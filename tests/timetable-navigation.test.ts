...
test("Home and Schedule subject links use canonical subject and lesson class across RU/KZ/EN; teachers stay hidden",()=>{
  for(const locale of ["ru","kk","en"] as const){
    for(const child of [
      createElement(WeeklyLessonList,{lessons:[row],subjects}),
      ...
      assert.deepEqual(initialLibraryFilters(Object.fromEntries(url.searchParams),classes[0].id),
        {q:"",subject:row.subject_id,classId:row.class_id});
      ...
      assert.ok(filtered.length>1);assert.ok(filtered.every(book=>book.subject_id===row.subject_id&&book.class_id===row.class_id));
    }
  }
});
test("A timetable subject with no published catalog matches uses the normal localized Library empty state",()=>{
  const initial=initialLibraryFilters({subject:subjects[2].id,classId:classes[0].id});
